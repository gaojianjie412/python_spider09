...
from u17.mongo_models import Comic, ComicChapter
...
class U17PyMysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item.collection == 'comic':
            ...
            sql = 'insert into comic_chapter (comic_id, chapter_id, chapter_url, title) values (%s, %s, %s, %s)'
            param = (item['comic_id'], item['chapter_id'], item['chapter_url'], item['title'])
...
# mongoengine保存
class U17PyMongoEnginePipeline(object):
    def __init__(self, uri, database):
        self.uri = uri
        self.database = database
    ...
    def process_item(self, item, spider):
        if item.colletion == 'comic':
            ...
            comic_detail = ComicChapter()
            comic_detail.comic_id = item['comic_id']
            comic_detail.chapter_id = item['chapter_id']
            comic_detail.chapter_url = item['chapter_url']
            comic_detail.title = item['title']
            comic_detail.save()
