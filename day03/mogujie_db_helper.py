...
def execute_sql2(db, cursor, product):
    sql = 'insert into products (trade_item_id, item_type, img, client_url, link, title, orgprice, price) values (%s, %s, %s, %s, %s, %s, %s, %s)'

    cursor.execute(sql, (product['tradeItemId'], product['itemType'], product['img'], product['clientUrl'], product['link'], product['title'], product['orgPrice'], product['price']))
    db.commit()
